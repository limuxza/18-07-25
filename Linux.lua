...
function Linux.Create(config)
    ...
    function LinuxLib.Tab(config)
        ...
        -- CORREÇÃO: Guarda qual aba deve ser a padrão
    ...
    -- CORREÇÃO: Mover esta função para depois que todas as abas são criadas
    local function SelectDefaultTab()
        task.wait(0.1) -- Espera um pouco para garantir que todas as abas foram criadas
        
        if DefaultTab and Tabs[DefaultTab] then
            -- Primeiro, deseleciona todas as abas
            ...
            if defaultTab.TabAccentBar then
                TweenService:Create(defaultTab.TabAccentBar, TweenInfo.new(0.3), {Size = UDim2.new(0, 4, 0, 24)}):Play()
            end
        ...
        elseif tabOrder > 0 then
            -- Se não há aba padrão definida, seleciona a primeira
            CurrentTab = 1
            ...
            if firstTab then
                ...
                if firstTab.TabAccentBar then
                    TweenService:Create(firstTab.TabAccentBar, TweenInfo.new(0.3), {Size = UDim2.new(0, 4, 0, 24)}):Play()
                end
                AnimateTabSliders(1)
                CheckEmptyState()
            end
    ...
    -- CORREÇÃO: Criar a aba Settings apenas se necessário
    if config.ConfigSave ~= false then
        local SettingsTab = LinuxLib.Tab({
            Name = "Settings",
            Icon = "rbxassetid://10734950309",
            Enabled = true,
            Default = false -- Importante: garantir que Settings nunca seja padrão
        })
    ...
    -- CORREÇÃO: Chamar a função para selecionar a aba padrão após todas as abas serem criadas
    task.spawn(SelectDefaultTab)
